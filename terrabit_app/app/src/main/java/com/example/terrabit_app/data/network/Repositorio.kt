...
class Repositorio @Inject constructor(
    ...
    private val bovinoDao = database.bovinoDao()
    ...
    suspend fun searchBovinosLocal(query: String): List<Animal> = withContext(Dispatchers.IO) {
        ...
        else bovinoDao.searchBovinos(query).map { it.toAnimal() }
    }
    ...
    suspend fun getDescargaGuiasMobilitat(
        nif: String, passwordMobilitat: String, codiMo: String, codiRega: String, dataSortida: String
    ...
    suspend fun getConfirmacionMovimientos(
        nif: String, passwordMobilitat: String, explotacioDestinacio: String, dataSortida: String
    ) = apiInterface.getConfirmacionMovimientos(nif, passwordMobilitat, explotacioDestinacio, dataSortida)

    suspend fun putRegistrarMuerte(request: RegistroMuerteBovi) = apiInterface.putRegistrarMuerte(request)
    suspend fun putRegistrarNacimiento(request: RegistroNacimientoBovi) = apiInterface.putRegistrarNacimiento(request)
    suspend fun putMoficarAnimal(request: PetModicarAnimal) = apiInterface.putMoficarAnimal(request)
    suspend fun putAltaGuia(request: PeticionAltaGuia) = apiInterface.putAltaGuia(request)
    suspend fun putModificarGuia(request: PeticionModificarGuia) = apiInterface.putModificarGuia(request)
    suspend fun putConfirmarMovi(request: PetConfirmacionMovi) = apiInterface.putConfirmarMovi(request)
    suspend fun putRegistroIntercanvio(request: PetRegistroIntercanvi) = apiInterface.putRegistroIntercanvio(request)
    suspend fun putMoficarMovi(request: PetModificacioMovi) = apiInterface.putMoficarMovi(request)
    suspend fun putIdentificacionPendiente(request: PetIdentificacion) = apiInterface.putIdentificacionPendiente(request)
    suspend fun putSolicitudDuplicado(request: PetSolicitudDuplicado) = apiInterface.putSolicitudDuplicado(request)
    suspend fun putSolicitudMaterial(request: PetSolicitudMaterial) = apiInterface.putSolicitudMaterial(request)

    // ------------------- Porcinos -------------------

    suspend fun altaGuiaPorcinas(request: AltaMovimientoGTR) = apiInterface.altaMovimientoPorcino(request)
